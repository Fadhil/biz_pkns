class User < ActiveRecord::Base
  ...
  has_and_belongs_to_many :roles, :join_table => :roles_users#, :join_table => ::RolesUsers.table_name
  ...
  def add_role(name)
    raise ArgumentException, "Role should be the name of the role not a role object." if name.is_a?(::Role)
    roles << ::Role[name] unless has_role?(name)
  end

  def has_role?(name)
    raise ArgumentException, "Role should be the name of the role not a role object." if name.is_a?(::Role)
    roles.any?{|r| r.name == name.to_s.camelize}
  end
